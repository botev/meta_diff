...
fn parse_g1<'input>(input: &'input str, state: &mut ParseState, pos: usize)
 -> RuleResult<OperatorType> {
    {
        let choice_res =
            {
                let start_pos = pos;
                {
                    let seq_res = parse_GTE(input, state, pos);
                    match seq_res {
                        Matched(pos, _) => {
                            {
                                let _context = state.context.clone();
                ...
                let choice_res =
                    {
                        let start_pos = pos;
                        {
                            let seq_res = parse_GT(input, state, pos);
                            match seq_res {
                                Matched(pos, _) => {
                                    {
                                        let _context = state.context.clone();
...
fn parse_expression<'input>(input: &'input str, state: &mut ParseState,
                            pos: usize) -> RuleResult<usize> {
    {
        let start_pos = pos;
        {
            let seq_res = parse_e1(input, state, pos);
            match seq_res {
                Matched(pos, first) => {
                    {
                        let seq_res =
                                                                    ...
                                                                    let result =
                                                                        match *graph_res
                                                                            ...
                                                                            match second
                                                                                ...
                                                                                Some(tuple)
                                                                                =>
                                                                                match graph.add_operation(tuple.0,
                                                                                                          vec!(first
                                                                                                               ,
                                                                                                               tuple
                                                                                                               .
...
fn parse_LTE<'input>(input: &'input str, state: &mut ParseState, pos: usize)
 -> RuleResult<()> {
    slice_eq(input, state, pos, "<=".to_string())
}
fn parse_GTE<'input>(input: &'input str, state: &mut ParseState, pos: usize)
 -> RuleResult<()> {
    slice_eq(input, state, pos, ">=".to_string())
}
fn parse_LT<'input>(input: &'input str, state: &mut ParseState, pos: usize)
 -> RuleResult<()> {
    slice_eq(input, state, pos, "<".to_string())
}
fn parse_GT<'input>(input: &'input str, state: &mut ParseState, pos: usize)
 -> RuleResult<()> {
    slice_eq(input, state, pos, ">".to_string())
}
